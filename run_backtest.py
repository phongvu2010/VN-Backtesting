...
def main():
    parser = argparse.ArgumentParser(
        description="Chương trình Backtesting Chứng khoán Việt Nam (VN-Backtest)"
    )
    parser.add_argument(
        "--ticker",
        type=str,
        default="FPT",
        help="Mã cổ phiếu cần backtest (mặc định: FPT)",
    )
    parser.add_argument(
        "--start", type=str, default="2020-01-01", help="Ngày bắt đầu (YYYY-MM-DD)"
    )
    parser.add_argument(
        "--end", type=str, default="2026-06-01", help="Ngày kết thúc (YYYY-MM-DD)"
    )
    parser.add_argument(
        "--cash",
        type=float,
        default=100_000_000.0,
        help="Số vốn ban đầu (mặc định: 100M VND)",
    )
    parser.add_argument(
        "--exchange",
        type=str.lower,
        ...
        help="Sàn giao dịch để áp biên độ trần/sàn (mặc định: hose)",
    )
    parser.add_argument(
        "--t_settle",
        type=int,
        default=2,
        help="Chu kỳ thanh toán cổ phiếu (mặc định: T+2)",
    )
    parser.add_argument(
        "--lot_size",
        type=int,
        default=100,
        help="Lô giao dịch tối thiểu (mặc định: 100)",
    )
    parser.add_argument(
        "--fee",
        type=float,
        default=0.0015,
        help="Phí giao dịch mua/bán (mặc định: 0.15%%)",
    )
    parser.add_argument(
        "--tax",
        type=float,
        default=0.001,
        help="Thuế bán chứng khoán (mặc định: 0.1%%)",
    )
    parser.add_argument(
        "--no_cache",
        action="store_true",
        help="Không sử dụng cache dữ liệu, tải mới hoàn toàn",
    )
    parser.add_argument(
        "--no_dynamic",
        action="store_true",
        help="Vô hiệu hóa quy tắc lịch sử động (chạy tĩnh)",
    )
    parser.add_argument(
        "--allow_odd_lot",
        action="store_true",
        help="Cho phép giao dịch lô lẻ (1 cổ phiếu)",
    )
    parser.add_argument(
        "--max_vol_ratio",
        type=float,
        default=None,
        help="Tỷ lệ khối lượng giao dịch tối đa so với Volume ngày (ví dụ: 0.1)",
    )
    parser.add_argument(
        "--exclude_auto_close",
        action="store_true",
        help="Loại bỏ các giao dịch tất toán cuối kỳ khỏi thống kê giao dịch",
    )
    parser.add_argument(
        "--adjust_corp_actions",
        action="store_true",
        help="Kích hoạt mô phỏng cổ tức/chia tách (chỉ dùng nếu giá đầu vào chưa điều chỉnh)",
    )
    parser.add_argument(
        "--margin_ratio",
        type=float,
        default=1.0,
        help="Tỷ lệ ký quỹ (ví dụ: 0.5 là đòn bẩy 2x, mặc định: 1.0 - không margin)",
    )
    parser.add_argument(
        "--margin_interest",
        type=float,
        default=0.13,
        help="Lãi suất margin năm (mặc định: 13%%)",
    )
    parser.add_argument(
        "--margin_maintenance",
        type=float,
        default=0.35,
        help="Tỷ lệ ký quỹ duy trì giải chấp (mặc định: 35%%)",
    )
    parser.add_argument(
        "--rf_rate",
        type=float,
        default=0.04,
        help="Lãi suất phi rủi ro năm (mặc định: 4%%)",
    )
    parser.add_argument(
        "--force_adjusted",
        type=str.lower,
        ...
        help="Ép buộc trạng thái điều chỉnh giá (auto: tự động phát hiện, true: đã điều chỉnh, false: chưa điều chỉnh)",
    )
    parser.add_argument(
        "--stop_loss",
        type=float,
        default=None,
        help="Tỷ lệ cắt lỗ Stop Loss (ví dụ: 0.07 cho 7%%)",
    )
    parser.add_argument(
        "--trailing_stop",
        type=float,
        default=None,
        help="Tỷ lệ chặn lãi Trailing Stop (ví dụ: 0.1 cho 10%%)",
    )
    parser.add_argument(
        "--optimize",
        action="store_true",
        help="Chạy tối ưu hóa tham số chiến lược (Grid Search)",
    )
    parser.add_argument(
        "--rebalance_interval",
        type=int,
        default=None,
        help="Chu kỳ cơ cấu tỷ trọng danh mục theo số phiên (ví dụ: 20)",
    )
    parser.add_argument(
        "--n_jobs",
        type=int,
        default=-1,
        help="Số tiến trình chạy song song khi tối ưu hóa (mặc định: -1 - sử dụng hết CPU)",
    )
    parser.add_argument(
        "--execution_at",
        type=str.lower,
        default="open",
        choices=["open", "close", "average", "vwap", "hl2", "typical"],
        help="Mô hình giá khớp lệnh (open, close, average/vwap, hl2, typical)",
    )
    parser.add_argument(
        "--slippage",
        type=float,
        default=0.0,
        help="Tỷ lệ trượt giá cố định (ví dụ: 0.001 cho 0.1%%)",
    )
    parser.add_argument(
        "--market_impact",
        type=float,
        default=0.0,
        help="Hệ số tác động thị trường gây trượt giá động (ví dụ: 0.1)",
    )
    parser.add_argument(
        "--rights_listing_delay",
        type=int,
        default=90,
        help="Số ngày chờ giải tỏa cổ tức cổ phiếu/quyền mua (mặc định: 90 ngày)",
    )
    parser.add_argument(
        "--report_name",
        type=str,
        default=None,
        help="Tên file báo cáo HTML đầu ra (ví dụ: report_123.html)",
    )
    parser.add_argument(
        "--benchmark",
        type=str.upper,
        default="VNINDEX",
        choices=["VNINDEX", "VN30"],
        help="Mã chỉ số benchmark so sánh (VNINDEX hoặc VN30)",
    )
    parser.add_argument(
        "--offline",
        action="store_true",
        help="Chạy chế độ offline, không tải mới hay cập nhật từ API",
    )
    parser.add_argument(
        "--dividend_tax",
        type=float,
        default=0.05,
        help="Thuế TNCN nhận cổ tức tiền mặt & bán cổ phiếu thưởng (mặc định: 5%%)",
    )
    ...
    print(config_str)
    print(
        f"Chi phí: Phí GD {args.fee*100:.2f}%, Thuế bán {args.tax*100:.2f}%, Thuế cổ tức {args.dividend_tax*100:.1f}%"
    )
    print("=" * 60)
    print(
        "CẢNH BÁO: Dữ liệu tải từ vnstock có Survival Bias (chỉ có các cổ phiếu còn hoạt động)."
    )
    print(
        "Để kiểm thử các cổ phiếu đã hủy niêm yết (ROS, FLC,...), vui lòng đặt tệp CSV/Parquet vào 'local_data/'."
    )
    print("=" * 60)
    ...
    for ticker in tickers:
        ...
        try:
            ...
            print(
                f"   Đã tải {len(df)} phiên giao dịch cổ phiếu {ticker} (Sàn: {exchanges[ticker].upper()})."
            )
    ...
    try:
        ...
        if not benchmark_data.empty:
            ...
            if bench_start > req_start or bench_end < req_end:
                print(
                    f"   CẢNH BÁO: Giai đoạn dữ liệu {args.benchmark} ({bench_start.strftime('%Y-%m-%d')} -> {bench_end.strftime('%Y-%m-%d')}) "
        ...
        print(
            f"CẢNH BÁO: Không thể tải {args.benchmark} làm benchmark ({e}). Sẽ không so sánh với benchmark."
        )
    ...
    if args.adjust_corp_actions:
        ...
        for ticker in tickers:
            try:
                events = loader.fetch_corporate_actions(
    ...
    if args.optimize:
        ...
        engine_kwargs = {
            ...
            "force_adjusted": (
                None
                if args.force_adjusted == "auto"
                else (args.force_adjusted == "true")
            ),
        ...
        if args.report_name:
            ...
            report_filename = (
                f"report_opt_{datetime.now().strftime('%Y%m%d_%H%M%S')}.html"
            )
    ...
    engine = BacktestEngine(
        ...
        adjust_corporate_actions=args.adjust_corp_actions,
        force_adjusted=(
            None if args.force_adjusted == "auto" else (args.force_adjusted == "true")
        ),
        margin_ratio=args.margin_ratio,
        margin_interest_rate=args.margin_interest,
        margin_maintenance_ratio=args.margin_maintenance,
    ...
    if benchmark_data is not None:
        print("-" * 60)
        print(
            f"Lợi nhuận {args.benchmark}     : {metrics['benchmark_return']*100:.2f}%"
        )
    ...
    if args.report_name:
        ...
        report_filename = (
            f"report_portfolio_{datetime.now().strftime('%Y%m%d_%H%M%S')}.html"
        )
