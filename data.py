...
class VNStockDataLoader:
    ...
    def _standardize_and_scale(
        self, df: pd.DataFrame, symbol: str, is_index: bool
    ) -> pd.DataFrame:
        df = df.copy()
    ...
    def fetch_data(
        ...
        if self.local_db_dir and os.path.exists(self.local_db_dir):
            ...
            if os.path.exists(parquet_path):
                try:
                    ...
                    print(
                        f"CẢNH BÁO: Không thể đọc tệp Parquet cục bộ {parquet_path} ({e})"
                    )
        ...
        if self.offline:
            if df_local is not None and not df_local.empty:
                ...
                return ensure_tz_naive(
                    df_local.loc[pd.to_datetime(start_date) : pd.to_datetime(end_date)]
                )
            ...
            raise RuntimeError(
                f"Chế độ Offline: Không tìm thấy dữ liệu cho {symbol} tại {self.local_db_dir} hoặc cache."
            )
        ...
        if df_local is not None and not df_local.empty:
            ...
            if days_diff > 30:
                print(
                    f"   Dữ liệu cục bộ của {symbol} đã cũ ({last_date.strftime('%Y-%m-%d')}). Đang tự động tải bù từ API..."
                )
                try:
                    ...
                    if is_index:
                        raw_df = self.market.index(symbol).ohlcv(
                            start=api_start, end=api_end, resolution="1D", count=5000
                        )
                    else:
                    ...
                    if raw_df is not None and not raw_df.empty:
                        ...
                        print(
                            f"   Cập nhật dữ liệu cho {symbol} thành công. Đã lưu vào bộ nhớ cache."
                        )
                    ...
                    print(
                        f"CẢNH BÁO: Không thể tải bù dữ liệu cho {symbol} ({e}). Tiếp tục chạy với dữ liệu cục bộ."
                    )
        ...
        try:
            ...
            if is_index:
                raw_df = self.market.index(symbol).ohlcv(
                    start=api_start, end=api_end, resolution="1D", count=20000
                )
            else:
                raw_df = self.market.equity(symbol).ohlcv(
                    start=api_start, end=api_end, resolution="1D", count=20000
            ...
            if os.path.exists(cache_path):
                print(
                    f"CẢNH BÁO: Không thể tải dữ liệu mới cho {symbol} ({e}). Sử dụng dữ liệu cache cũ."
                )
                try:
                    df = pd.read_csv(cache_path, parse_dates=["Date"])
                    df.set_index("Date", inplace=True)
    ...
    def fetch_corporate_actions(
        ...
        if self.local_db_dir and os.path.exists(self.local_db_dir):
            ...
            if os.path.exists(local_event_path):
                try:
                    df_raw = pd.read_csv(
                        local_event_path,
                        parse_dates=["exright_date", "payout_date", "listing_date"],
                    )
                    df_local = ensure_cols_tz_naive(df_raw)
                    print(f"-> Đã tìm thấy tệp sự kiện cục bộ: {local_event_path}")
                except Exception as e:
                    print(
                        f"CẢNH BÁO: Không thể đọc tệp sự kiện cục bộ {local_event_path} ({e})"
                    )
        ...
        if use_cache and is_fresh:
            try:
                ...
                print(
                    f"CẢNH BÁO: Lỗi đọc cache sự kiện cho {symbol} ({e}). Sẽ tải mới."
                )
        ...
        try:
            ...
            if self.local_db_dir and os.path.exists(self.local_db_dir):
                local_event_path = os.path.join(
                    self.local_db_dir, f"events_{symbol}.csv"
                )
            ...
            print("=" * 80)
            print(
                f"CẢNH BÁO: Không thể kết nối API vnstock để lấy sự kiện doanh nghiệp của {symbol}."
            )
    ...
    def fetch_exchange_map(self, use_cache: bool = True) -> dict[str, str]:
        ...
        for source in ["KBS", "VCI", "MSN"]:
            try:
                ...
                if (
                    df_symbols is not None
                    and not df_symbols.empty
                    and "symbol" in df_symbols.columns
                    and "exchange" in df_symbols.columns
                ):
