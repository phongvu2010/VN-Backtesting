...
def run_backtest():
    ...
    try:
        proc = subprocess.Popen(
            cmd, stdout=log_file, stderr=log_file, cwd=ROOT_DIR, text=True
        )
        ...
        return jsonify(
            {
                "status": "completed",
                "ticker": task["ticker"],
                "note": "Không tìm thấy file báo cáo mới sinh ra.",
                "optimize": task["optimize"],
            }
        )
    else:
        return jsonify(
            {"status": "failed", "ticker": task["ticker"], "error_code": poll}
        )
