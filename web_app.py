...
def get_reports():
    ...
    reports_path = os.path.join(ROOT_DIR, 'reports', 'report_portfolio_*.html')
    ...
    for f in files:
        basename = os.path.basename(f)
        # format: report_portfolio_YYYYMMDD_HHMMSS.html
        # Extract time for display
        time_str = "Unknown"
        try:
            parts = basename.replace('report_portfolio_', '').replace('.html', '').split('_')
            if len(parts) >= 2:
                dt_str = parts[0] + parts[1] # YYYYMMDDHHMMSS
